...
const Footer = () => {
  ...
  // Theme-based colors
  const bgColor = theme === 'dark' ? 'bg-[#181818]' : 'bg-gray-100';
  const textColor = theme === 'dark' ? 'text-white' : 'text-gray-800';
  const secondaryTextColor = theme === 'dark' ? 'text-gray-300' : 'text-gray-600';
  const borderColor = theme === 'dark' ? 'border-gray-700' : 'border-gray-200';
  const inputBgColor = theme === 'dark' ? 'bg-gray-700' : 'bg-white';
  const inputTextColor = theme === 'dark' ? 'text-white' : 'text-gray-900';
  const buttonBgColor = theme === 'dark' ? 'bg-[#B8FF8C]' : 'bg-[#B8FF8C]';
  const buttonTextColor = theme === 'dark' ? 'text-gray-900' : 'text-white';
  ...
  return (
    <footer className={`pt-10 ${bgColor}`}>
      {/* Changed max-width from max-w-screen-xl to max-w-5xl (or max-w-[1200px] for custom width) */}
      <div className="max-w-5xl mx-auto px-4 md:px-8">
          ...
          <div className="flex-1 mt-6 md:mt-0">
            ...
            >
              <div className="relative">
                <svg
                  className={`w-6 h-6 ${secondaryTextColor} absolute left-3 inset-y-0 my-auto`}
                ...
                <input
                  type="email"
                  required
                  placeholder="Any feed back ...."
                  className={`w-full pl-12 pr-3 py-2 ${inputTextColor} ${inputBgColor} outline-none border ${borderColor} focus:border-[#B8FF8C] shadow-sm rounded-lg`}
                />
              </div>
              <button className={`block w-auto py-3 px-4 font-medium text-sm text-center ${buttonTextColor} ${buttonBgColor} hover:opacity-90 active:opacity-80 rounded-lg shadow`}>
                Send
              </button>
            </form>
          </div>
        ...
        <div className="flex-1 mt-16 space-y-6 justify-between sm:flex md:space-y-0">
          {footerNavs.map((item, idx) => (
            <ul className="space-y-4" key={idx}>
              <h4 className={`${textColor} font-semibold sm:pb-2`}>
                {item.label}
              </h4>
              {item.items.map((el, idx) => (
                <li key={idx}>
                  <a
                    href={el.href}
                    className={`${secondaryTextColor} duration-150 hover:text-[#B8FF8C]`}
                  >
                    {el.name}
                  </a>
                </li>
        ...
        <div className={`mt-10 py-10 border-t ${borderColor} items-center justify-between sm:flex`}>
          <p className={secondaryTextColor}>
            © {new Date().getFullYear()} Nael Production. All rights reserved.
          </p>
          <div className="flex items-center gap-x-6 mt-6">
            <a href="#" className={secondaryTextColor}>
              <svg
                className="w-6 h-6 hover:text-[#B8FF8C] duration-150"
                fill="none"
                viewBox="0 0 48 48"
              >
                <g clipPath="url(#a)">
